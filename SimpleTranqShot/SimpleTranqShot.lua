...
function STranq_SendRotation()
	...
	local syncper, syncnum, syncgroups = strrep("¼",per), strrep("½",num), strrep("¾",groups);
...
function STranq_SetupRotation(msg)
	local _,_,perGroup,maxShown,numGroups = string.find( msg, "(¼+)" );
	_,_,maxShown = string.find(msg, "(½+)" );
	_,_,numGroups = string.find(msg, "(¾+)" );
	if ( perGroup ) then
	...
	for name in string.gfind(msg, "¦(%a+)°") do
		if ( raid[name] ) then
			if ( not hunters ) then
				hunters = {};
			end
